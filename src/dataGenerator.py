...
def get_coloring_steps():
    ...
    pgraph_vertices, pgraph_adjacency_lists = createPointGraph(
        points, triangles)
    tgraph_vertices, tgraph_adjacency_lists = createTriangleGraph(triangles)

    pgraph_colors, coloring_steps = colorPointGraph(
        pgraph_vertices, tgraph_vertices, tgraph_adjacency_lists)

    lower_bound, minimal_combinations = reduceUpperBound(
        pgraph_colors, visibility_sets)
